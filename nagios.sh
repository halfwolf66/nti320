...
setenforce 0  #Turn off SElinux so it doesn't trip on us
...
sed -i 's,allowed_hosts=127.0.0.1,allowed_hosts=127.0.0.1\, 10.128.0.46\/20,g' /etc/nagios/nrpe.config
#above enables connections from your subnet, need to adjust subnet

sed -i 's,dont_blame_nrpe=0,dont_blame_nrpe=1,g /etc/nagios.cfg
...
#Spin up a machine called example-a with all the nrpe plugins installed and properly configured path to
#nagios
#put its ip address in the address column below
...
define service{
 ...
 define service{
          ...
          chec_command                    check_http
          notifications_enabled           0
          }
#Define a service to check the disk space of the root partition
#on the example-a machine. Warning if < 20% free, critical
...
#users on the example-a machine. Warning if > 20 users, critical if
#> 50 users.
define service{
        use                   generic-service                   ; Name of service template to use
        host_name             example-a
